...
Each audit-time check also requires `receiptRef` and `receiptSha256` identifying its exact targeted runner receipt under `.vibe/receipts`. Copy argv, outcome, timestamps, fingerprints and coverage from that receipt; the validator checks all fields and the captured log hash. Audit coverage includes the managed-operation IDs being proved, as well as AC/gate IDs. Old inline-only audit checks require a new audit attempt and completing checks; never rewrite old audit evidence to add a binding. Legacy ledgers without durable request/read hashes require an inspected checkpoint, not reinitialization.
