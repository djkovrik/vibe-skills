...
For each obligation/surface, only the latest current receipt counts; a later failure wins over an earlier success. Invocation-local ValidationContext reads each receipt once and fingerprints each unique scope once. Targeted checks use reviewed transitive scopes; integration checks use global content identity. A commit with unchanged content does not invalidate evidence. Historical receipt/log hashes and imported handoff hashes remain checked; historical symbols are not current acceptance anchors.

A closureManifest binds the current immutable audit/request hashes, AppSpec/workspace identity, and the audit's successful integration receipts. There is no mandatory final-kind execution. The auditor independently inspects reused checks and executes missing/suspect checks. Binding never marks ACs verified or checkpoints source drift.
...
implementation-complete requires all ACs and applicable repository gates resolved, current successful surface evidence, a valid closure manifest, a fresh PASS audit, current fingerprints, and report parity. release-ready additionally resolves every applicable platform, external, and release gate and permits no blocked-external.
...
## Local acceptance and validation cost

`locallyVerified` requires current independent PASS and complete available surfaces; `implementationComplete` still requires every AC and repository gate, and `releaseReady` every applicable release/platform gate. Request-bound unavailablePairs permit `locally-verified` audit obligations; they never make an AC globally verified. Repository failures cannot be classified as unavailable.

Resume is a safety/recovery operation, not aggregate closure. Readiness rejects cheap blockers before receipt traversal. Closure validates receipt hashes, latest outcomes, live evidence, audit and report parity. No persistent cache or daemon is required. Evidence replacement uses the flow contract's reconcile-evidence operation; never delete history to make validation pass.
