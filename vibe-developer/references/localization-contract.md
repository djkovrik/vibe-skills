...
- Keep translations in Compose Multiplatform Resources `strings.xml` files under the locale-appropriate `composeResources/values*` directories. Use one stable resource key in every supported locale from `app.locales`; new locales add another resource set without changing domain or persistence schemas.
