...
Compile and run affected behavioral tests before handoff. Render-smoke a production preview during foundation: reject zero discovery, require actual snapshots and production resources, and cover light/dark plus RU at 200% font scale before expanding the matrix. Record, inspect and verify affected goldens at stable screen/package boundaries; every applicable primary state still needs light/dark coverage. Select additional locale/font/device stress combinations by risk and pairwise coverage. Deferred surfaces remain unverified. After readiness and freeze, run one integration matrix, then obtain an independent audit that verifies and reuses current receipts. Bind its evidence in the closure manifest without a mandatory post-audit rerun.
