...
1. At the beginning of every turn and immediately after any context compaction, interruption, or uncertainty about retained state, if `.vibe/delivery-ledger.json` exists, run read-only `scripts/resume-delivery.py` before relying on prior conversation. Follow its active AC, drift classification, pending hand-offs, blockers, and mandatory next action. Preserve expected drift for reconciliation; do not edit on unexpected drift. If no ledger exists, strictly validate an approved AppSpec 2.0 with `scripts/validate-app-spec.py --require-current`. Any 1.x artifact is an unsupported protocol; do not migrate, delete, or reinitialize it automatically.
...
11. Run targeted checks first. During implementation on Windows, this orchestrator is the only Gradle owner and uses `scripts/run-gradle.ps1` to write structured Protocol 2.0 receipts. A receipt names exact argv/tasks, covered obligation/surface pairs, UTC timestamps, start/end fingerprints, execution status, real exit code, and log hash. A timeout leaves interrupted evidence. Use `run-check.py` for non-Gradle checks. Implementation specialists request commands but never invoke Gradle themselves. Once implementation is stopped for a fresh audit, transfer the exclusive verification lease to that auditor, which may use the same serialized runner for audit-time checks.
12. Checkpoint before boundary expansion, after hand-off ingestion, before and after long checks, and before every turn ends. Preserve the durable request, decisions and exact pending actions in `execution.requiredReads`; specialists checkpoint unfinished assignments as specified in the shared recovery contract. After each milestone reread AppSpec and the ledger. Import only inspected immutable hand-offs with `ingest-handoff.py`; conversation is not evidence. Preserve every unfinished obligation as an individual status.
...
Treat `.vibe/delivery-ledger.json` as the only editable delivery-state source and mutate it atomically only with the previous digest. Generated reports are projections; hand-offs, receipts, the audit request, and the audit are immutable files. Record material user steering immediately with `record-decision.py` from a captured exact user message. A waiver requires an accepted decision JSON scoped to the exact obligation with captured user approval; arbitrary file links are invalid. `blocked-external` is legal only for platform/external/release gates and never with `release-ready`. Any AppSpec file or workspace fingerprint change makes completion evidence stale. Adopt an approved revision only with `reconcile-spec.py`; never reset the ledger to discard history. Commit before the final verification sequence; after a commit, reconcile its HEAD change explicitly and rerun affected evidence.
