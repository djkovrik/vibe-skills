...
- For monetized apps, require Yandex Mobile Ads as the preferred/default provider and verify geography-aware CMP applicability, required-form-only UX, persisted consent state, privacy-before-init, and blocked initialization/requests in GDPR-applicable or unresolved states. Accept Google UMP as a standalone CMP without adding Google Mobile Ads/AdMob SDK.
