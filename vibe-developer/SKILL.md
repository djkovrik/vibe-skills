...
1. At the beginning of every turn and immediately after any context compaction, interruption, or uncertainty about retained state, if `.vibe/delivery-ledger.json` exists, run read-only `scripts/resume-delivery.py --compact` before relying on prior conversation. Follow its active AC, drift classification, pending hand-offs, blockers, and mandatory next action. Preserve expected drift for reconciliation; do not edit on unexpected drift. If no ledger exists, strictly validate an approved AppSpec 2.0 with `scripts/validate-app-spec.py --require-current`. Any 1.x artifact is an unsupported protocol; do not migrate, delete, or reinitialize it automatically.
...
7. Detect installed `vibe-*`, Compose Expert, Lazyweb and image-generation capabilities. For new delivery planning or any asset work, apply the [asset contract](../vibe-assets-creator/references/asset-contract.md): Product Designer specifies the complete icon/logo/illustration inventory; `$vibe-assets-creator` creates or generates missing artwork and supplies Compose Resources plus delivery evidence. Require the current asset inventory at intake; planned generation is valid intake, not a request for the user to draw assets.
8. Before the first code edit, run `scripts/init-delivery-ledger.py`, compare the shared canonical inventory with normative prose, save the actual request and constraints in a repository file, then use `checkpoint-delivery.py --request-file <saved-request>` with the current ledger digest, active AC, owner, boundaries, phase, and exact next action. Add specialist contracts with `--required-read`; normative AppSpec prose and active FLOW/SCREEN inputs are included automatically. Build dependency-aware capability/flow packages containing related ACs; AC IDs remain separate acceptance obligations. Use `delivery-work.py` JSON requests to register package boundaries, reviewed input scopes and assignment-local baselines. Assign one owner per non-overlapping change; a skill route need not create another agent. Preserve the immutable JSON contract in [specialist-handoff-contract.md](references/specialist-handoff-contract.md). Keep all existing component, Store, Manager, preview, DI-module, localization, UI/golden, privacy, ads, and CI deliverables explicit.
9. Execute necessary concerns together inside each capability package, sequencing internal dependencies. Connect the real production root and a reachable save/read/restart flow in the first package; integrate each later flow before switching packages. Request compilation and targeted tests before final specialist handoff. Track integrated flows separately from verified ACs; close each AC only after its own production/UI/platform behavior and required verification surfaces are complete:

```text
approved AppSpec 2.0 -> ledger initialization and checkpoint
-> capability packages with early production integration, assignment-local hand-offs and targeted receipts
...
11. Run targeted checks first. During implementation on Windows, this orchestrator is the only Gradle owner and uses `scripts/run-gradle.ps1` to write structured Protocol 2.0 receipts. A receipt names exact argv/tasks, covered obligation/surface pairs, UTC timestamps, start/end fingerprints, execution status, real exit code, and log hash. A timeout leaves interrupted evidence. Use `run-check.py` for non-Gradle checks. Implementation specialists request commands as soon as code is ready to check, before final handoff, but never invoke Gradle themselves. Use `run-gradle.ps1 -RequestPath` or `delivery-work.py` check requests to preserve argument arrays. Targeted receipts require a registered, orchestrator-reviewed transitive input scope; register a conservative whole-repository scope when dependencies are uncertain. Use explicit `kind: integration` for global integration/audit checks and `kind: final` for post-audit closure. Compile changes immediately, run combined relevant suites at capability boundaries, and reserve the full matrix for final integration. Once implementation is stopped for a fresh audit, transfer the exclusive verification lease to that auditor, which may use the same serialized runner for audit-time checks.
12. Automate baseline capture, handoff generation, ingest/checkpoint and receipt binding with `delivery-work.py`. Checkpoint at assignment start, meaningful result, completed checks and before interruption/return. Use feature globs rather than per-file administrative steps. Inspect unexpected drift; independent registered owners may edit concurrently without changing each other's assignment baselines. Preserve durable requests, unresolved checks and exact next action; resume compactly and reread changed/lost inputs. After two failures of the same category, diagnose the shared cause before repeating the loop. Conversation alone is not evidence.
13. When every local entry is closed, checkpoint, create immutable `audit-request.json` with `create-audit-request.py`, stop implementation, and run `run-acceptance-audit.py` to launch `$vibe-acceptance-auditor` in a fresh ephemeral process. Each attempt lives under `.vibe/audits/<request-id>/`; preserve prior attempts. The launcher records the host thread event and command metadata; self-declared context fields alone do not prove isolation. On `GAPS`, resume fixes and require a new request and clean audit. On `BLOCKED`, preserve the exact blocker. Without a separate process and valid launch receipt, do not claim completion. If the CLI is unavailable, preserve a tooling blocker; do not fabricate launch evidence.
14. After fresh `PASS`, run one explicit `final` receipt covering every verified obligation and verification surface, atomically bind it into the ledger, render both reports, and invoke `validate-delivery-ledger.py` once. That validator performs strict AppSpec, inventory, receipt ordering, audit, and report-parity checks and reports `implementation-complete` and `release-ready` separately.
...
Treat `.vibe/delivery-ledger.json` as the only editable delivery-state source and mutate it atomically only with the previous digest. Generated reports are projections; hand-offs, receipts, the audit request, and the audit are immutable files. Record material user steering immediately with `record-decision.py` from a captured exact user message. A waiver requires an accepted decision JSON scoped to the exact obligation with captured user approval; arbitrary file links are invalid. `blocked-external` is legal only for platform/external/release gates and never with `release-ready`. Any AppSpec file or workspace fingerprint change makes final completion evidence stale; intermediate targeted evidence is invalidated by its registered input scope, not unrelated files. Adopt an approved revision only with `reconcile-spec.py`; never reset the ledger to discard history. Commit before the final verification sequence; after a commit, reconcile its HEAD change explicitly and rerun affected evidence.
