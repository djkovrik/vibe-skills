...
8. Decide `icon`, `text`, `icon + text`, or `intentionally no icon`; inventory all icons, logos and illustrations in AppSpec `assetRequirements` and link ASSET IDs from design/screens. Specify semantics, variants, intended dp size, palette/tint, transparency and acquisition using the [asset contract](../vibe-assets-creator/references/asset-contract.md). Route actual creation/generation to `$vibe-assets-creator` before the affected UI slice is verified. An approved creation brief may be `planned`; ask only about unresolved material choices or exact unavailable external brand inputs, preserving existing user authorization.
...
- Prefer canonical Material Symbols for standard actions when consistent with the approved design; keep one style and coherent fill/weight behavior. Assets Creator produces original custom artwork or adapts permitted sources and records provenance; exact third-party identity must follow supplied/authorized brand material. Do not replace clear text with icon-only controls merely to increase icon count.
