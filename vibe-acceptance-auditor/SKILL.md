...
For deliveries using capability packages, inspect their entry-point/navigation/restart evidence and actual production reachability independently. AC counts and component tests alone do not prove flow integration. Intermediate scoped receipts do not replace the globally current audit-time/final evidence required by the closure contract.
