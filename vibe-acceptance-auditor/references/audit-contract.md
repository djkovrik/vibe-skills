...
Every check must include `receiptRef` (directly under `.vibe/receipts`) and `receiptSha256`. It must exactly match a targeted runner receipt's argv, exit status, execution status, start/end fingerprints, times and obligation/surface coverage; equivalent timezone offsets are allowed. The receipt's log must exist and match its SHA-256. Include managed-operation IDs in runner coverage when proving them. Inline fields without this binding cannot prove execution; old audits require a new request and new checks, never editing prior artifacts.
